...
https://github.com/StareInTheAir/mpv-trakt-sync-daemon/archive/master.zip
...
1. Change directory to your install location: `cd %APPDATA%\mpv-trakt-sync-daemon`
1. `pip install virtualenv`
1. `virtualenv venv` (virtual environment created in `venv` folder)
1. `venc\Scripts\activate.bat` (virtual environment activated)
...
1. Hold the `Alt` key down and drag `win-launch.bat` from the install location Explorer to the autostart Explorer to create a shortcut in the autostart directory
...
- Once mpv is closing, requests can no longer be sent to it. To keep track of your playback position requests need to be sent in regular intervals, so that when mpv quit the last known state can be used for determining your playback state.
